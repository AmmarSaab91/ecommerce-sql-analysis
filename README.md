...
1. distribution_centers: Contains warehouse or distribution center information.
2. users: Contains customer profile and acquisition details such as country, city, and traffic source.
3. products: Contains product-level information such as category, brand, department, retail price, and cost.
4. orders: Contains order-level records with timestamps and order status.
5. events: Contains user activity and behavioral events such as product views, cart actions, and purchases.
6. inventory_items: Contains item-level inventory and sale information.
7. order_items: Links orders, products, users, and inventory items at the line-item level.
---
Project Structure
1) Schema Setup:
...
distribution_centers.sql, users.sql, products.sql, orders.sql, events.sql, inventory_items.sql,  order_items.sql
Purpose:
...
Files:
recency.sql, frequency_analysis.sql, monetary_segmentation.sql, basic_rfm_classification.sql
...
Files:
revenue_metrics.sql, profitability_analysis.sql, order_level_kpi.sql
What this section measures:
...
Files:
funnel_analysis.sql, session_analysis.sql, traffic_source_performence.sql
What this section measures:
...
Files:
delay-bottleneck-detection.sql, processing_time_analysis.sql, status_distribution.sql
What this section measures:
...
1. Run the schema setup files.
2. Run data quality checks.
3. Review duplicates and missing values.
4. Run customer segmentation analysis.
5. Run revenue and profitability analysis.
6. Run user behavior and acquisition analysis.
7. Run operational analysis.
